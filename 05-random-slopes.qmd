...
```{r}
lq2002.2 <- lq2002.1 %>%
  mutate(., #Grand mean center the level-2 variable
         gtsig_cent = GTSIG - mean(GTSIG)) %>%
  group_by(COMPID) %>%
         mutate(., #Group mean center the level-1 variable
...
## Step three: use `ggeffects` to obtain more easily interpretable results

Interpreting a significant interaction from a coefficient alone is hard. The `ggeffects` package helps by turning the model into predicted values you can plot directly, rather than asking you to reason about what a coefficient on a coefficient means. We will use it to see predicted hostility across `TSIG`, separately for companies at three representative levels of `GTSIG`.

```{r}
#| label: fig-ggpredict
#| fig-cap: "Predicted hostility across individual task significance, at three levels of company task significance."
#| message: false
...
Read this the same way you would read any other predicted-value plot in this book: the vertical axis is predicted `HOSTILE`, not a coefficient, and each of the three lines traces predicted hostility across `tsig_cent` at one fixed level of `gtsig_cent` (a below-average, average, and above-average company). If the three lines were parallel, `TSIG` and `GTSIG` would not interact — an individual soldier's own sense of task significance would predict hostility the same way no matter the company. That is not what happens here. The lines are clearly separated on the left and converge on the right: among soldiers who feel their own work is *not* very significant, company climate matters a lot, with predicted hostility ranging from about 1.6 in low-`GTSIG` companies down to about 1.0 in high-`GTSIG` companies. Among soldiers who already feel their own work is highly significant, that company-level gap nearly disappears, and all three lines converge toward the same, low predicted hostility. Put in plain language: a company's overall sense that the work matters seems to compensate for an individual soldier not feeling that way personally, but has little left to add once a soldier already feels that way on their own.
